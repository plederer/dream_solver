...
rho_0 = rho_inf
u_0 = u_inf
p_0 = p_inf * (1 + AMP * exp(-(x**2 + y**2)/beta**2))
...
class Pulse(Benchmark):
    ...
    def add_meta_data(self, cfg: SolverConfiguration):
        cfg.info["Sound Domain"] = (f"W: {self.sound.W}, H: {self.sound.H}")
        cfg.info["Dezibel Reduction"] = self.dB
        cfg.info["Grid Factor"] = self.grid_factor
        cfg.info["Grid Function X"] = repr(self.grid_x)
        cfg.info["Grid Function Y"] = repr(self.grid_y)
    ...
    def set_boundary_conditions(self, solver: CompressibleHDGSolver):
        solver.boundary_conditions.set(bcs.Outflow(p_inf), "left|right|bottom|top")
    ...
    def draw_scenes(self, solver: CompressibleHDGSolver):
        solver.drawer.draw()
        solver.drawer.draw_acoustic_pressure(p_inf, autoscale=False, min=-1e-2, max=1e-2)
...
class PSponge(Pulse):
    ...
    def add_meta_data(self, cfg: SolverConfiguration):
        super().add_meta_data(cfg)
        cfg.info["\u03A0-Sponge Orders"] = self.order
        cfg.info["\u03A0-Sponge Lengths X"] = self.lengths_x
        cfg.info["\u03A0-Sponge Lengths Y"] = self.lengths_y
        cfg.info["\u03A0-Sponge Function X"] = self.sponge_x
        cfg.info["\u03A0-Sponge Function Y"] = self.sponge_y
        cfg.info["\u03A0-Sponge Weights X"] = self.weights_x
        cfg.info["\u03A0-Sponge Weights Y"] = self.weights_y

    def set_domain_conditions(self, solver: CompressibleHDGSolver):
        super().set_domain_conditions(solver)
        ...
        for order, ring in zip(self.order, rings):
            ...
            # sigma_x = SpongeWeight.quadratic(lx, cfg.Mach_number.Get(), dB=-40)
            # sigma_y = SpongeWeight.quadratic(ly, cfg.Mach_number.Get(), dB=-40)
            ...
            solver.domain_conditions.set(dcs.PSpongeLayer(
...
class Sponge(Pulse):
    ...
    def add_meta_data(self, cfg: SolverConfiguration):
        super().add_meta_data(cfg)
        cfg.info["\u03A0-Sponge Length X"] = self.length_x
        cfg.info["\u03A0-Sponge Length Y"] = self.length_y
        cfg.info["\u03A0-Sponge Function X"] = self.sponge_x
        cfg.info["\u03A0-Sponge Function Y"] = self.sponge_y
        cfg.info["\u03A0-Sponge Weight X"] = self.weight_x
        cfg.info["\u03A0-Sponge Weight Y"] = self.weight_y
    ...
    def set_domain_conditions(self, solver: CompressibleHDGSolver):
        ...
        for ring in rings:
            solver.domain_conditions.set(dcs.SpongeLayer(farfield, sponge_x, sponge_y), domain=ring.mat)
