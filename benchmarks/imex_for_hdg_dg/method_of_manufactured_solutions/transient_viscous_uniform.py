...
from config import (TRANSIENT_CFG,
                        STAGE_TO_SCHEME,
                        NavierStokesMMS,
                        get_uniform_meshes_with_x_split,
                        get_gassner_mms,
...
output = f"transient_mms_uniform/navier-stokes/{USER['splitting']}/{USER['stage']}stage_scheme"
...
GENERAL['time.timer.interval'] = (0.0, 0.01)
...
if __name__ == "__main__":
    ...
    mesh, imp_mesh, exp_mesh = get_uniform_meshes_with_x_split(32, 32, periodic=periodic)

    if USER['splitting'] == 'implicit':
        simulation = NavierStokesMMS(HDG, get_gassner_mms, f'hdg_{USER["viscous_treatment"]}', periods=1, periodic=periodic)
        transient_convergence_routine(mesh, simulation, time_steps=USER['time_steps'])
    elif USER['splitting'] == 'explicit':
        simulation = NavierStokesMMS(DG, get_gassner_mms, 'dg_ip', periods=1, periodic=periodic)
        transient_convergence_routine(mesh, simulation, time_steps=USER['time_steps'])
    else:
        hdg = NavierStokesMMS(HDG, get_gassner_mms, f'hdg_{USER["viscous_treatment"]}', domain="implicit", boundaries="top|bottom", periods=1, periodic=periodic)
        dg = NavierStokesMMS(DG, get_gassner_mms, f'dg_{USER["viscous_treatment"]}', domain="explicit", boundaries="left|top|right|bottom", periods=1, periodic=periodic)
