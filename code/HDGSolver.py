...
class compressibleHDGsolver():
    ...
    def SetUp(self, force = CoefficientFunction((0,0,0,0)), condense = False):
        ...
        # q = nabla u 
        self.a += ( InnerProduct(q,r) + InnerProduct(u,div(r))).Compile() * dx
        self.a += -InnerProduct(uhat,r*n).Compile() * dx(element_boundary = True)


        # konv flux
        self.a += -InnerProduct(self.FU.Flux(u),grad(v)).Compile() * dx()

        self.a += InnerProduct(self.FU.numFlux(uhat,u,uhat,n),v).Compile() * dx(element_boundary = True)
        self.a += (1-psi) * InnerProduct(self.FU.numFlux(uhat,u,uhat,n),vhat).Compile() * dx(element_boundary = True)
        
        # diff flux

        self.a += -InnerProduct(self.FU.diffFlux(u,q),grad(v)).Compile() * dx()
        self.a += InnerProduct(self.FU.numdiffFlux(u,uhat,q,n),v).Compile() * dx(element_boundary = True)
        self.a += (1-psi) * InnerProduct(self.FU.numdiffFlux(u,uhat,q,n),vhat).Compile() * dx(element_boundary = True)
        ...
        #self.a += (   (self.bnd_data-uhat) * vhat) * ds(skeleton = True, definedon = self.mesh.Boundaries(self.dirichlet))
        #self.a += ( psi *   (self.bnd_data-uhat) * vhat) * dx(element_boundary = True) #ds(skeleton = True, definedon = self.mesh.Boundaries(self.dirichlet))
        self.a += ( (self.bnd_data-uhat.Trace()) * vhat.Trace()) * ds(definedon = self.mesh.Boundaries(self.dirichlet))
        
        #a += (   (u-uhat) * vhat) * ds(skeleton = True, definedon = mesh.Boundaries("left|right|top|bottom"))

        self.f = LinearForm(self.fes)
        self.f+= InnerProduct(force,v) * dx
        self.f.Assemble()
    ...
    def Solve(self):                       
        ...
        solver.Solve(maxit=10, maxerr=1e-11, dampfactor=1,
        ...
        for it in range(5):
            ...
            res.data += self.f.vec
            inv = self.a.mat.Inverse(self.fes.FreeDofs(), inverse = "pardiso") 
            w.data = inv*res
            self.gfu.vec.data -= w
            if sqrt(InnerProduct (w,res)**2) < 1e-20:
                break
        '''
        #################################################################################
    ...
    def temperature(self):        
        return self.ff_data["gamma"] * self.ff_data["Minf"]**2 * self.pressure / self.gfu.components[0][0]
