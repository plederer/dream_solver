...
def mms_routine(func):
    ...
    def polyomial_order_routine(cfg: CompressibleFlowSolver, simulation: EulerMMS | NavierStokesMMS, order: int, level: int):
    ...
    def simulation_routine(cfg: CompressibleFlowSolver, level, simulation: EulerMMS | NavierStokesMMS):
        ...
        for order in ORDERS:
            ...
            Uh = polyomial_order_routine(cfg, simulation, order, level)
...
with open("mms_hdg_errors.csv", "w", newline="") as f:
...
plt.savefig("mms_hdg_errors.pdf")
plt.savefig("mms_hdg_errors.svg")
