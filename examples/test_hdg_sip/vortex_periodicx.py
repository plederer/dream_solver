...
from dream.compressible import CompressibleFlowSolver, flowfields, FarField, Initial, Outflow, GRCBC, NSCBC, AdiabaticWall, IsothermalWall
from dream.compressible.conservative import ConservativeDG
...
for bc, edge in zip(['bottom', 'right', 'top', 'left'], face.edges):
    edge.name = bc
if periodicx:
    periodic_edge = face.edges[1]
    periodic_edge.Identify(face.edges[3], "periodicx", IdentificationType.PERIODIC)

mesh = Mesh(OCCGeometry(face, dim=2).GenerateMesh(maxh=0.2, quad_dominated=True))
...
use_sdg_ip = isinstance(cfg.fem, ConservativeDG)
if not use_sdg_ip:
    use_hdg_ip = True
    use_hdg_mixed = not use_hdg_ip
    if use_hdg_ip:
        filename = "hdg_ip"
        ...
        raise NotImplementedError("Current option is not considered yet.")
else:
    filename = "sdg_ip"
    cfg.fem.viscous_treatment = "interior_penalty"
    cfg.fem.viscous_treatment.interior_penalty_coefficient = 10.0
...
#cfg.bcs['top|bottom'] = IsothermalWall(temperature=2700)

if periodicx:
    cfg.bcs['right|left'] = "periodic"
else:
