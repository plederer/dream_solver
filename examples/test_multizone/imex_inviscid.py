...
def init_hdg_cfg(cfg, time_info, multizone_time, nPoly, plot_sol_vtk=False):
    ...
    # Set up VTK stream.
    init_vtk_stream(cfg, plot_sol_vtk, "hdg")



# Function that sets the SDG configuration.
def init_sdg_cfg(cfg, time_info, multizone_time, nPoly, plot_sol_vtk=False):
    ...
    #cfg.fem.scheme = "rk_ars22"
    #cfg.fem.scheme = "rk_ars33"
    cfg.fem.scheme = "rk_ars43"
    cfg.time.timer.interval = (time_info.t0, time_info.tf)
    cfg.time.timer.step = time_info.dt
    ...
    # Set up VTK stream.
    init_vtk_stream(cfg, plot_sol_vtk, "sdg")
...
def set_interface_conditions(cfg_imp, cfg_exp):
    ...
    gfu_imp = cfg_imp.get_solution_fields()
    gfu_exp = cfg_exp.get_solution_fields()
...
# Time information.
time_info = TimeInfo( t0=0.0, tf=4.0, dt=0.04 )
...
init_hdg_cfg(cfg_hdg, time_info, multizone_time, nPoly, plot_sol_vtk=True)
init_sdg_cfg(cfg_sdg, time_info, multizone_time, nPoly, plot_sol_vtk=True)
