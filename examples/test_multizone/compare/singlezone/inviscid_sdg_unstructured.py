...
def create_simple_grid(ne, lx, ly):
    ...
    #domain = WorkPlane().MoveTo(2, 0).RectangleC(lx, ly).Face()
    ...
    domain0 = WorkPlane().MoveTo(2, 0).RectangleC(lx, ly).Face()
    domain0.name = "internal"
    domain0.edges[0].name = 'bottom'
    domain0.edges[1].name = 'interface'
    domain0.edges[2].name = 'top'
    domain0.edges[3].name = 'left'
    
    # Subdomain: will end up explicit.
    domain1 = WorkPlane().MoveTo(dx1+1, 0).RectangleC(lx1+2, ly).Face()
    domain1.name = "internal"
    domain1.edges[0].name = 'bottom'
    domain1.edges[1].name = 'right'
    domain1.edges[2].name = 'top'
    ...
    geo = OCCGeometry(Glue([domain0, domain1]), dim=2)

    # Deduce the grid spacing.
    maxh = min( lx, ly )/ne

    # Generate the mesh.
    mesh = ngs.Mesh(geo.GenerateMesh(maxh=maxh))#, quad_dominated=True))
...
def init_vtk_stream(cfg, plot_sol_vtk=False, filename="output"):
    if plot_sol_vtk:
        ...
        cfg.io.vtk.path = "inviscid_unstructured"
