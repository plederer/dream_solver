...
def CreateSimpleGrid(ne, lx, ly):
    ...
    bottom = domain.edges[0]
    bottom.name = 'bottom'
    right = domain.edges[1]
    right.name = 'right'
    top = domain.edges[2]
    top.name = 'top'
    left = domain.edges[3]
...
U0 = transportfields()
U0.u = get_analytic_solution(cfg.time.timer.interval[0], cfg.diffusion_coefficient)
...
def gaussian_hill_routine(label):
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            # cfg.io.draw({"u": Uh.u})
            # cfg.io.draw({"Exact[u]": Uex.u},  min=0.0, max=10.0)
            # cfg.io.draw({"Diff[u]": (ue - uh)}, min=-0.1, max=0.1)
            fields = cfg.fem.get_fields()
            fields["Exact[u]"] = ue
            fields["Diff[u]"] = ue - uh
            cfg.io.vtk.fields = fields
            cfg.io.vtk.enable = False
            cfg.io.vtk.rate = 10
