...
def abs(x: ngs.CF | float) -> ngs.CF | float:
...
def max(x: ngs.CF | float, y: ngs.CF | float = 0) -> ngs.CF | float:
...
def min(x: ngs.CF | float, y: ngs.CF | float = 0) -> ngs.CF | float:
...
def interval(x: ngs.CF | float, start: ngs.CF | float, end: ngs.CF | float):
    return min(max(x, start), end)


def trace(x: ngs.CF) -> ngs.CF | float:
...
def unit_vector(x: ngs.CF | Sequence | Generator) -> ngs.CF:
    x = as_vector(x)
    return x/ngs.sqrt(inner(x, x))


def inner(x: ngs.CF | ngs.CF, y: ngs.CF | ngs.CF) -> ngs.CF | float:
    return ngs.InnerProduct(x, y)


def outer(x: ngs.CF, y: ngs.CF) -> ngs.CF:
    x = as_vector(x)
    y = as_vector(y)
    return ngs.OuterProduct(x, y)


def as_scalar(x: ngs.CF | float) -> ngs.CF | float:
    if isinstance(x, ngs.CF):
        return x
    elif isinstance(x, Number):
        return ngs.CF(x)
...
def is_scalar(x: ngs.CF | float) -> bool:
...
def as_vector(x: ngs.CF | Sequence | Generator) -> ngs.CF:
    if isinstance(x, ngs.CF):
        return x
...
def as_matrix(x: ngs.CF | Sequence | Generator, dims: tuple[int, ...] | None = None) -> ngs.CF:
...
def is_zero(x: ngs.CF | float | ngs.CF | ngs.CF) -> bool:
    if isinstance(x, Number):
        return x == 0.0
    elif isinstance(x, ngs.CF):
