...
class MixedMethod(InterfaceConfiguration, is_interface=True):
    ...
    def add_mixed_formulation(self, 
...
class StrainHeat(MixedMethod):
    ...
    def add_mixed_formulation(self, 
        ...
        blfi['mixed'] -= ngs.InnerProduct(Uhat.u,
...
class ConservativeMethod(InterfaceConfiguration, is_interface=True):
    ...
    def add_time_derivative_formulation(self, 
                                        blfi: dict[str, ngs.comp.SumOfIntegrals],
                                        blfe: dict[str, ngs.comp.SumOfIntegrals],
                                        lf:   dict[str, ngs.comp.SumOfIntegrals]):
...
class DG(ConservativeMethod):
    ...
    # TODO: must ensure that,
    # 1) explicit in time
    # 2) euler/convection only
    ...
    def add_convection_formulation(self, 
                                   blfi: dict[str, ngs.comp.SumOfIntegrals],
                                   blfe: dict[str, ngs.comp.SumOfIntegrals],
                                   lf:   dict[str, ngs.comp.SumOfIntegrals]):
...
class HDG(ConservativeMethod):
    ...
    def set_initial_conditions(self, U: ngs.CF = None):
        ...
        blfi  = ngs.BilinearForm(self.Uhat_gfu.space)
        blfi += u * v * ngs.dx(element_boundary=True)
        ...
        with ngs.TaskManager():
            ...
            self.Uhat_gfu.vec.data = blfi.mat.Inverse(inverse="sparsecholesky") * f.vec
    ...
    def add_diffusion_formulation(self, 
                                  blfi: dict[str, ngs.comp.SumOfIntegrals],
                                  blfe: dict[str, ngs.comp.SumOfIntegrals], 
                                  lf:   dict[str, ngs.comp.SumOfIntegrals]):
...
class ConservativeFiniteElementMethod(CompressibleFiniteElement):
    ...
    def add_symbolic_forms(self, 
        ...
        self.method.add_time_derivative_formulation(blfi, blfe, lf)
        self.method.add_convection_formulation(blfi, blfe, lf)

        if not self.cfg.pde.dynamic_viscosity.is_inviscid:
            self.method.add_diffusion_formulation(blfi, blfe, lf)

        self.mixed_method.add_mixed_formulation(blfi, blfe, lf)
